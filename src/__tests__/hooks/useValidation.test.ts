...
const mockValidateTable = validateTable as jest.MockedFunction<typeof validateTable>;
const mockBuildValidationSummary = buildValidationSummary as jest.MockedFunction<typeof buildValidationSummary>;
...
describe('useValidation', () => {
  const mockTableData: TableData = [
    {
      rowIndex: 0,
      cells: {
        proceso: { value: 'Proceso 1', originalValue: 'Proceso 1', isEdited: false, hasError: false, columnName: 'proceso', rowIndex: 0 },
        actividad: { value: 'Actividad 1', originalValue: 'Actividad 1', isEdited: false, hasError: false, columnName: 'actividad', rowIndex: 0 }
      },
      hasError: false,
      isEdited: false
    }
  ...
  describe('validate', () => {
    it('should validate table successfully', async () => {
      ...
      await act(async () => {
        validationResult = await result.current.validate(mockTableData, mockParsedData);
      });
    ...
    it('should call validateTable with correct arguments', async () => {
      ...
      // Assert
      expect(mockValidateTable).toHaveBeenCalledWith(mockTableData, mockParsedData);
    });
    ...
    it('should handle large table data', async () => {
      ...
      const largeTableData: TableData = Array.from({ length: 100 }, (_, i) => ({
        rowIndex: i,
        cells: {
          proceso: { value: `Proceso ${i}`, originalValue: `Proceso ${i}`, isEdited: false, hasError: false, columnName: 'proceso', rowIndex: i }
        },
        hasError: false,
        isEdited: false
      })) as any;
      ...
      const differentTableData: TableData = [
        {
          rowIndex: 0,
          cells: {
            proceso: { value: 'Different Proceso', originalValue: 'Different Proceso', isEdited: false, hasError: false, columnName: 'proceso', rowIndex: 0 }
          },
          hasError: false,
          isEdited: false
        }
