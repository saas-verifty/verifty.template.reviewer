...
const mockGetMaxFileSizeMB = getMaxFileSizeMB as jest.MockedFunction<typeof getMaxFileSizeMB>;
...
describe('useFileUpload', () => {
  ...
  describe('File validation', () => {
    ...
    it('should reject file exceeding size limit', async () => {
      ...
      // Assert
      expect(result.current.error).toBe(ERROR_MESSAGES.FILE_SIZE_EXCEEDED(maxSizeMB));
      expect(result.current.file).toBeNull();
      expect(result.current.data).toBeNull();
      expect(mockParseExcelFile).not.toHaveBeenCalled();
    });
    ...
    it('should accept file name with at least 5 parts separated by underscore', async () => {
      // Arrange
      const { result } = renderHook(() => useFileUpload());
      const validFile = createMockFile('part1_part2_part3_part4_part5.xlsx', 1024);
      mockParseExcelFile.mockResolvedValueOnce(mockParsedData);
