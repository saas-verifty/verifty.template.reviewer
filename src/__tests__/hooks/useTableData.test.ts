...
describe('useTableData', () => {
  ...
  describe('updateCell', () => {
    ...
    it('should not mark cell as edited when value equals original', () => {
      ...
      // Get original value
      const originalValue = result.current.table[0].cells.proceso
        .value as string
    ...
    it('should preserve originalValue', () => {
      ...
      // Assert
      expect(result.current.table[0].cells.proceso.originalValue).toBe(
        originalValue
      )
      expect(result.current.table[0].cells.proceso.value).toBe('Nuevo Proceso')
    })
    ...
    it('should only update specified row', () => {
      ...
      expect(result.current.table[1].cells.proceso.value).toBe(
        row1OriginalValue
      )
    ...
    it('should only update specified column', () => {
      ...
      const actividadOriginalValue =
        result.current.table[0].cells.actividad.value
      ...
      expect(result.current.table[0].cells.actividad.value).toBe(
        actividadOriginalValue
      )
    })
