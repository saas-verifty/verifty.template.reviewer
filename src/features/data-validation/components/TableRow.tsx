...
function getOptionsForColumn(
  ...
  if (CATALOG_FIELDS[columnName]) {
    const key = CATALOG_FIELDS[columnName]
      .key as keyof ParsedExcelData['validationSheet']['data'][0]
  ...
  const catalogKey = HAZARD_TYPE_TO_CATALOG_KEY[
    tipoPeligro
  ] as keyof ParsedExcelData['hazardCatalog']
  ...
  if (columnName === 'descripcion_peligro') {
    const values = hazardRows
      .map((r) => r.descripcion_peligro)
      .filter((v) => !!v && v.trim() !== '')
    return [...new Set(values)]
  }
  ...
  if (columnName === 'descripcion_especifica_peligro') {
    const descPeligro = String(
      row.cells['descripcion_peligro']?.value ?? ''
    ).trim()
    if (descPeligro) {
      hazardRows = hazardRows.filter(
        (r) => r.descripcion_peligro === descPeligro
      )
    }
  ...
  if (columnName === 'consecuencia_efecto_posible') {
    ...
    const descEspecifica = String(
      row.cells['descripcion_especifica_peligro']?.value ?? ''
    ).trim()
    ...
    if (descEspecifica === UI_MESSAGES.EMPTY_VALUE || descEspecifica === '') {
      hazardRows = hazardRows.filter(
        (r) =>
          !r.descripcion_especifica_peligro ||
          r.descripcion_especifica_peligro.trim() === ''
      )
    } else if (descEspecifica) {
      hazardRows = hazardRows.filter(
        (r) => r.descripcion_especifica_peligro === descEspecifica
      )
    }

    const values = hazardRows
      .map((r) => r.consecuencia_efecto_posible)
      .filter((v) => !!v && v.trim() !== '')
    return [...new Set(values)]
  }
        ...
        if (options) {
          return (
            <SelectCell
              ...
              options={options}
              onChange={(value) =>
                onChange(row.rowIndex, cell.columnName, value)
              }
            />
          )
        }
