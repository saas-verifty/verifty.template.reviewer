...
export const ERROR_MESSAGES = {
  ...
  FILE_NAME_INVALID: 'El nombre del archivo no cumple con el formato requerido.',
  ...
  EXCEL_PARSE_ERROR: (error: string) => `Error al parsear el archivo Excel: ${error}`,
  EXCEL_PARSE_UNKNOWN_ERROR: 'Error al parsear el archivo Excel: Error desconocido',

  // Validation Errors
  PROCESS_REQUIRED_FOR_CHILDREN:
    'La actividad, subactividad o peligro no pueden existir sin proceso.',
  ACTIVITY_REQUIRED_FOR_CHILDREN: 'La subactividad o peligro no pueden existir sin actividad.',
  SUBACTIVITY_REQUIRED_FOR_DANGER: 'El peligro no puede existir sin subactividad.',

  // JSON Generation
  JSON_GENERATION_NO_DATA: 'No se pudo generar el JSON: no hay datos válidos',

  // S3 Upload Errors
  AWS_NOT_CONFIGURED: 'AWS no está configurado. Por favor, configura las variables de entorno.',
  AWS_NOT_CONFIGURED_CHECK: 'AWS no está configurado. Verifica las variables de entorno en .env',
